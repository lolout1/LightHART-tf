...
class BaseTrainer:
    def __init__(self, arg):
        ...
        self.print_log(f"Model parameters: {num_params:,} ({num_params/(1024**2):.2f} MB)")
    ...
    def _load_model(self):
        if self.arg.phase == 'train':
            ...
            input_shapes = {
                'accelerometer': (self.arg.model_args.get('acc_frames', 128), 
                                 self.arg.model_args.get('acc_coords', 4))
            }
            if 'skeleton' in self.arg.dataset_args.get('modalities', []):
                input_shapes['skeleton'] = (self.arg.model_args.get('acc_frames', 128), 32, 3)
    ...
    def cal_metrics(self, targets, predictions):
        ...
        try:
            ...
            if len(np.unique(targets)) < 2 or len(np.unique(predictions)) < 2:
                ...
                return accuracy, 0.0, 0.0, 0.0, 50.0
