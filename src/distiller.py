...
from utils.visualization import plot_confusion_matrix, plot_loss_curves, plot_distribution
...
class DistillationTrainer:
    ...
    def load_teacher_model(self):
        """Load teacher model"""
        # Correct the import to match the actual file name
        ...
        if hasattr(self.args, 'teacher_weight') and self.args.teacher_weight:
            teacher_weight = f"{self.args.teacher_weight}_{self.test_subject[0]}.pth" if self.test_subject else self.args.teacher_weight
    ...
    # Add a custom distillation loss
    def distillation_loss(self, y_true, student_logits, teacher_logits, student_features, teacher_features):
        ...
        # Soft target loss (KL divergence on features)
        # First, apply softmax with temperature
        teacher_probs = tf.nn.softmax(teacher_features / temperature, axis=-1)
        student_probs = tf.nn.softmax(student_features / temperature, axis=-1)
    ...
    def load_data(self):
        ...
        try:
            ...
            if self.args.phase == 'train':
                ...
                self.norm_train = split_by_subjects_tf(builder, self.train_subjects, self.args.distill_args.get('fuse', False))
                ...
                try:
                    plot_distribution(self.norm_train['labels'], self.work_dir, 'train', logger.info)
                except Exception as e:
                    logger.warning(f"Error visualizing distribution: {e}")
                
                if self.val_subject:
                    logger.info(f"Processing validation data for subjects: {self.val_subject}")
                    self.norm_val = split_by_subjects_tf(builder, self.val_subject, self.args.distill_args.get('fuse', False))
                    ...
                    try:
                        plot_distribution(self.norm_val['labels'], self.work_dir, 'val', logger.info)
                    except Exception as e:
                        logger.warning(f"Error visualizing distribution: {e}")
    ...
    def start(self):
        ...
        if self.args.phase == 'train':
            ...
            val_subjects = [38, 46]  # Standard validation subjects
