#!/bin/bash
# train_student.sh - Train the student model with accelerometer data
...
echo "Starting student model training at $(date)"
...
# Fix the model path and update parameters
...
if [ ${TRAINING_STATUS} -eq 0 ]; then
  ...
  # Export to TFLite
  echo "Exporting best student model to TFLite..."
  python -c "
import os
import glob
import sys
...
"
  ...
  echo "Student model training failed with status ${TRAINING_STATUS}"
  exit ${TRAINING_STATUS}
fi

echo "Student model training completed. Output directory: ${WORK_DIR}"
