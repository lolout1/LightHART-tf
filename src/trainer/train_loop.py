...
def train_epoch(model, optimizer, criterion, data_loader, epoch, num_epochs, logger=None):
    ...
    # Always print starting message
    print(f"Training epoch {epoch+1}/{num_epochs} - {total_batches} batches")
    if logger:
        logger(f"Training epoch {epoch+1}/{num_epochs} - {total_batches} batches")

    # Skip tqdm and implement custom progress reporting
    for batch_idx, (inputs, targets, _) in enumerate(data_loader):
        # Log progress every 10 batches
        if batch_idx % 10 == 0:
            progress_msg = f"Training epoch {epoch+1}/{num_epochs}: batch {batch_idx+1}/{total_batches}"
            print(progress_msg)
            if logger:
                logger(progress_msg)
    ...
    # Log completion - always print to stdout even if logger is None
...
def evaluate_model(model, criterion, data_loader, epoch=0, name='val', logger=None):
    ...
    # Always print starting message
    print(f"Evaluating {name} (epoch {epoch+1}) - {total_batches} batches")
    if logger:
        logger(f"Evaluating {name} (epoch {epoch+1}) - {total_batches} batches")

    # Skip tqdm and implement custom progress reporting
    for batch_idx, (inputs, targets, _) in enumerate(data_loader):
        # Log progress every 5 batches
        if batch_idx % 5 == 0:
            progress_msg = f"Evaluating {name} (epoch {epoch+1}): batch {batch_idx+1}/{total_batches}"
            print(progress_msg)
            if logger:
                logger(progress_msg)
