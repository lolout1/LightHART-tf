...
def csvloader(file_path, **kwargs):
    """Load CSV data - matches PyTorch implementation"""
    try:
        ...
        if 'skeleton' in file_path:
            ...
            cols = 3  # Accelerometer/gyroscope have 3 channels
...
class UTD_MM_TF(tf.keras.utils.Sequence):
    """TensorFlow version of UTD_mm dataset loader"""
    ...
    def _validate_data(self):
        ...
        if self.skl_data is not None and len(self.skl_data) > 0:
            ...
            if len(self.skl_data.shape) == 3:
                ...
                if self.skl_features == 96:  # 32 joints * 3
    ...
    def cal_smv(self, sample):
        """Calculate Signal Magnitude Vector - matches PyTorch implementation"""
...
class DatasetBuilder:
    ...
    def process(self, data, label):
        ...
        try:
            if self.mode == 'avg_pool':
                ...
                # Use peak detection for fall detection
    ...
    def make_dataset(self, subjects, fuse):
        ...
        for trial in self.dataset.matched_trials:
            if trial.subject_id in subjects:
                ...
                if self.task == 'fd':
                    label = int(trial.action_id > 9)  # Fall detection
                elif self.task == 'age':
                    label = int(trial.subject_id < 29 or trial.subject_id > 46)
                else:
                    label = trial.action_id - 1  # HAR
    ...
    def normalization(self, acc_mean=None, acc_std=None, skl_mean=None, skl_std=None, compute_stats_only=False):
        """Normalize data using consistent statistics"""
        ...
        for key, value in self.data.items():
            ...
            # Use provided statistics or compute new ones
...
def prepare_smartfallmm_tf(arg):
    ...
    root_paths = [
        ...
        '/path/to/data/smartfallmm'  # Add your actual data path
    ]
...
def split_by_subjects_tf(builder, subjects, fuse=False, compute_stats_only=False, 
    ...
    """Split dataset by subjects with consistent normalization"""
