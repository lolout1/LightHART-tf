...
class UTD_MM_TF(tf.keras.utils.Sequence):
    def __init__(self, dataset, batch_size, modalities=['accelerometer'], shuffle=True):
        """
        Initialize the UTD_MM_TF dataset class.

        Args:
            dataset (dict): Dictionary containing the dataset with keys like 'accelerometer', 'skeleton', 'labels'.
            batch_size (int): Number of samples per batch.
            modalities (list): List of modalities to include (e.g., ['accelerometer'], ['skeleton'], or both).
            shuffle (bool): Whether to shuffle the data at initialization and after each epoch.
        """
        self.dataset = dataset
        ...
        # Validate and load data based on requested modalities
        self.data = {}
        if 'accelerometer' in self.modalities:
            acc_data = dataset.get('accelerometer', dataset.get('acc_data_with_smv', dataset.get('acc_data')))
            if acc_data is None:
                raise ValueError("Accelerometer data is required but not found in the dataset.")
            self.data['accelerometer'] = tf.convert_to_tensor(acc_data, dtype=tf.float32)
            self._compute_smv()  # Compute SMV if accelerometer data is included
        if 'skeleton' in self.modalities:
            skl_data = dataset.get('skeleton', dataset.get('skl_data'))
            if skl_data is None:
                raise ValueError("Skeleton data is required but not found in the dataset.")
            # Handle various skeleton data shapes robustly
            skl_tensor = tf.convert_to_tensor(skl_data, dtype=tf.float32)
            if len(skl_tensor.shape) == 3:
                if skl_tensor.shape[2] == 96:
                    # Reshape (samples, frames, 96) to (samples, frames, 32, 3)
                    skl_tensor = tf.reshape(skl_tensor, [-1, skl_tensor.shape[1], 32, 3])
                elif skl_tensor.shape[2] != 32:
                    raise ValueError(f"Invalid skeleton feature dimension: {skl_tensor.shape[2]}. Expected 32 or 96.")
            elif len(skl_tensor.shape) == 4:
                if skl_tensor.shape[2] != 32 or skl_tensor.shape[3] != 3:
                    raise ValueError(f"Invalid skeleton shape: {skl_tensor.shape}. Expected (samples, frames, 32, 3).")
            else:
                raise ValueError(f"Unexpected skeleton data shape: {skl_tensor.shape}")
            self.data['skeleton'] = skl_tensor

    def _compute_smv(self):
        """
        Compute Signal Magnitude Vector (SMV) for accelerometer data and concatenate it.
        """
    ...
    def __getitem__(self, idx):
        ...
        Returns:
            tuple: (data_dict, batch_labels, batch_indices)
                - data_dict: Dictionary with modality data (e.g., {'accelerometer': ..., 'skeleton': ...})
                - batch_labels: Tensor of labels for the batch
                - batch_indices: Tensor of indices for the batch
        """
        ...
        for modality in self.modalities:
            batch_data = tf.gather(self.data[modality], batch_indices)
            data[modality] = batch_data
